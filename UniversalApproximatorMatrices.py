...
# The neural network
from NeuralNetworkFunctions import neural_network as ua
...
class RestoreUniversalApproximator:
    def __init__ (weights_file, biases_file):
        self.weights = np.load(weights_file)
        self.biases = np.load(biases_file)
    ...
    def restore_NN (self, input_vector):
        ...
        # First hidden layer
        #z = np.matmul(self.weights[0],input_vector) + self.biases[0]
        ...
        return np.matmul(a,self.weights[n])
...
"""
    ...
    def get_eigenvalues (self, lst):
        dim = int(self.output_dim_sqrt)
        return np.sort(diag(reshape(lst[-1], (dim, dim))))
